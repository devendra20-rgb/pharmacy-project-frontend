  ...
  const renderDropdownContent = (menu) => {
    if (menu.isGrid) {
      return (
        <div
          className={`flex p-6 ${
            menu.isWellBeing ? "w-[550px]" : "w-[900px]"
          } justify-start gap-x-12 bg-white shadow-2xl border-t-[6px] border-teal-700 rounded-b-sm`}
        >
      ...
      <div className="w-60 bg-white shadow-2xl border-t-[6px] border-teal-700 rounded-b-sm">
        {menu.items.map((item, idx) => (
          <Link
            key={idx}
            href="#"
            className="block px-5 py-3 text-[13px] text-gray-700 font-medium hover:bg-teal-50 hover:text-teal-700 border-b border-gray-100 last:border-b-0 transition-colors"
          >
            {item}
          </Link>
        ))}
    ...
    <header className="bg-white border-b border-gray-200 sticky top-0 z-50 h-20 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">,
        ...
        <div className="flex items-center h-full">
          <Link href="/" className="mr-10 flex items-center">
            <span className="text-3xl font-black text-teal-700 tracking-tighter">
              Dev<span className="text-gray-900"></span>
            </span>
          </Link>
        ...
        <div className="flex items-center h-full">
          ...
          >
            {isSearchOpen ? (
              <form
                ...
                className="flex items-center w-full bg-white border-2 border-teal-600 rounded-sm h-10 px-2 shadow-sm"
              >
                <input
                  autoFocus
                  type="text"
                  placeholder="Search..."
                  className="flex-1 px-2 text-sm outline-none w-full font-medium"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
                <button type="submit" className="text-teal-700 p-1">
              ...
              <button
                ...
                className="p-2 hover:bg-teal-50 rounded-full text-gray-700 transition-colors"
              >
          ...
          {!isSearchOpen && (
            <div className="flex items-center">
              {/* 1. Vertical Divider Line - Separates Search from Login */}
              <div className="h-8 w-[1px] bg-gray-200 mx-6" />
              ...
              <div className="flex items-center space-x-8">
                ...
                {/* 3. Subscribe Button */}
                <Button className="bg-teal-600 hover:bg-teal-700 text-white font-black rounded-sm px-6 h-10 text-[12px] tracking-wider shadow-sm transition-all">
                  SUBSCRIBE
                </Button>
              </div>
            </div>
        ...
        <button
          ...
          className="lg:hidden ml-4 text-teal-700"
        >
